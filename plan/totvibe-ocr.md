...
- **Local dev orchestration**: a single root `justfile` delegates everything to `podman compose -f infra/compose.yaml -f infra/compose.dev.yaml up`. Four containerized services (wrangler dev, the transcription service, MinIO, vLLM), one entry point. Variants (mock service without GPU, future test stack) are sibling override files. See [`project-structure.md`](./board/done/001-proj-struct.md) for the repo layout.
