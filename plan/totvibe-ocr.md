...
- v0.1: Cloudflare-native, runnable locally via `wrangler dev` + podman; no auth, no public exposure. [DECIDED]
...
- **Four runnable pieces. No Postgres, no pgmq:**
  1. **TS edge layer** — TanStack Start app deployed as a Worker, with a single Durable Object class. Local dev via `wrangler dev` (DO + Worker runtime emulated by miniflare under the hood; default port `localhost:8787`). The DO is addressed by `USER_DO.idFromName('default')`.
...
```mermaid
flowchart LR
    Browser["🌐 Browser<br/>(SPA: React 19 + TanStack Router<br/>+ TanStack DB + jotai)"]
    ...
    subgraph Local["Laptop (v0.x) or cloud GPU host (v1)"]
        Pipe["Python pipeline<br/>FastAPI · asyncio · glmocr SDK<br/>PP-DocLayoutV3 (GPU)"]
        VLLM["vLLM<br/>GLM-OCR · GPU"]
    end

    Browser <-->|"HTTPS<br/>upload + hydrate"| W
    Browser <-->|"WSS · live deltas"| W
    W <-->|"RPC stub"| DO
...
- **Local dev orchestration**: a `justfile` brings up `wrangler dev` (TS+DO emulation), `podman compose up minio vllm`, and `uv run python pipeline.py`. Four runnable pieces, one entry point.
...
- Live updates: **hibernation-aware WebSocket** from the DO. Compatibility date `2026-04-07+`. [DECIDED]
- Reactive client store: **TanStack DB** collection with custom `sync` (initial fetch + WebSocket delta stream + exponential-backoff reconnect). [DECIDED]
...
- Local dev orchestration: **`justfile`** recipes — `just dev` (wrangler + MinIO + vLLM + pipeline), `just dev-edge` (wrangler only), `just dev-pipeline` (pipeline only), `just up-stateful` (MinIO + vLLM in podman). [DECIDED]

## 11. Roadmap
- **v0.1 / walking skeleton (local-only)**: full Cloudflare-native architecture runnable locally via `wrangler dev` + podman (MinIO + vLLM) + Python pipeline on the host. Singleton DO. No auth. **No Cloudflare deployment.** E2E test framework skeleton (vitest + React Testing Library) drives the local stack with a mock pipeline so the test loop has no GPU dependency. [DECIDED]
...
- Sharp edges of TanStack DB 0.6+ as the reactive client store — custom `sync` adapter shape, optimistic-mutation reconciliation when the DO's broadcast races the local optimistic insert.
